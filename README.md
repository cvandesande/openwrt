...
- vendor FMAN and DPAA dataplane ownership on Mono Gateway
- ASK runtime-state and control-path integration
- first true hardware-offload proof on a validated preferred 1G routed WAN
  path
- direct-routed production-path and reply-half proof on a validated 1G
  production path
...
- `OpenWRT-ASK` carries the acceleration stack primarily as a very large
  vendor patch import. Its two main kernel drops, `950-nxp-lsdk.patch` and
  `951-nxp-ask.patch`, total 222,509 lines and touch about 480 files.
- This fork keeps OpenWrt as the main integration repo and boxes the vendor
  stack behind a much narrower local integration layer. The current local
  kernel patch layer is 456 lines across 15 files, while the larger ASK
  sources are kept in pinned package repos and fetched through normal OpenWrt
  package recipes.
